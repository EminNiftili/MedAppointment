...
{
    internal class AdminUserMeStrategy : IUserMeResponseStrategy
    {
        private readonly ILogger<AdminUserMeStrategy> _logger;
        private readonly IUnitOfClient _unitOfClient;
        private readonly IPrivateClientInfoService _privateClientInfoService;

        public AdminUserMeStrategy(
            ILogger<AdminUserMeStrategy> logger,
        ...
        {
            return userTypes.Contains(UserType.SystemAdmin);
        }

        public async Task<Result<object>> BuildAsync(long userId)
        {
            _logger.LogTrace("Building admin user me response. UserId: {UserId}", userId);
            ...
            _logger.LogInformation("Admin user me response built. UserId: {UserId}", userId);
            return result;
        }
    }
}
